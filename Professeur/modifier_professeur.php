...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($matricule) && !empty($nom) && !empty($prenom) && !empty($email) && !empty($specialite)) {
        $result = modifier_professeur($matricule, $nom, $prenom, $email, $telephone, $specialite, $professeur_id);
        if ($result) {
            header("Location: index_professeurs.php?success=modified");
            exit();
        } else {
            $error = "Erreur lors de la modification du professeur.";
        }
    } else {
...
<head>
    ...
    <title>Modifier un Professeur - Centre de Formation</title>
                ...
                <div class="flex-grow-1">
                    <div class="container-fluid p-0" style="max-width: 800px;">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h3 class="fw-bold mb-0 text-dark">Modifier le professeur</h3>
                            ...
                            <div class="card-body p-4 p-md-5">
                                <form action="" method="post">
                                    <div class="mb-4">
                                        ...
                                        <input type="text" class="form-control form-control-lg bg-light border-0" name="matricule" id="matricule" value="<?= htmlspecialchars($professeur['matricule']) ?>" required>
                                    </div>

                                    <div class="row g-3">
                                        <div class="col-md-6 mb-3">
                                            <label for="nom" class="form-label fw-medium text-secondary">Nom *</label>
                                            <input type="text" class="form-control form-control-lg bg-light border-0" name="nom" id="nom" value="<?= htmlspecialchars($professeur['nom']) ?>" required>
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label for="prenom" class="form-label fw-medium text-secondary">Prénom *</label>
                                            <input type="text" class="form-control form-control-lg bg-light border-0" name="prenom" id="prenom" value="<?= htmlspecialchars($professeur['prenom']) ?>" required>
                                        </div>
                                    </div>

                                    <div class="mb-4">
                                        <label for="email" class="form-label fw-medium text-secondary">Email *</label>
                                        <input type="email" class="form-control form-control-lg bg-light border-0" name="email" id="email" value="<?= htmlspecialchars($professeur['email']) ?>" required>
                                    </div>

                                    <div class="mb-4">
                                        <label for="telephone" class="form-label fw-medium text-secondary">Téléphone</label>
                                        <input type="tel" class="form-control form-control-lg bg-light border-0" name="telephone" id="telephone" value="<?= htmlspecialchars($professeur['telephone']) ?>">
                                    </div>

                                    <div class="mb-4">
                                        <label for="specialite" class="form-label fw-medium text-secondary">Spécialité *</label>
                                        <input type="text" class="form-control form-control-lg bg-light border-0" name="specialite" id="specialite" value="<?= htmlspecialchars($professeur['specialite']) ?>" required>
                                    </div>
